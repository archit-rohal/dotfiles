...
while [ $image_count -lt $desired_image_count ]; do 
  response=$(curl -s -H "User-Agent: my-app" "https://www.reddit.com/r/wallpapers/.json?after=$after")
  urls=$(echo "$response" | jq -r '.data.children[] | select(.data.url_overridden_by_dest != null) | .data.url_overridden_by_dest' | grep -P 'https?://\S+\.(png|jpg|jpeg|gif)')
